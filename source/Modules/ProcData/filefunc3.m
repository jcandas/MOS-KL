...
switch parameters.data.normalize
    ...
    case 1, norm_tag = func2str(methods.all.normalizedata); %'Normalized';
end
...
% Results live in source/results (root set by running "paths" from source/).
% Override the root via parameters.data.resultsRoot to keep a run's output separate
% (e.g. a semisynthetic variant) without overwriting the main results tree.
root = getenv('PNAS_ROOT');
assert(~isempty(root), 'PNAS_ROOT not set -- run ''paths'' from the source folder first.');
rF = fullfile(root, 'results');
if isfield(parameters.data, 'resultsRoot') && ~isempty(parameters.data.resultsRoot)
    rF = parameters.data.resultsRoot;
end
