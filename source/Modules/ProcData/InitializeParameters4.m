...
parameters.synthetic.NKLTerms = ''; % KL Truncation for generating Semisynthetic Data
...
parameters.snapshots.k1 = []; % KL Truncation for Class A
...
parameters.multilevel.svmonly = []; % if 1 then SVM only, if 0 then multilevel, if 2 then Trajan's Multilevel, 
parameters.multilevel.splitTraining = []; %if True, then Training A data is split into ML-filter subset and SVM training subset
parameters.multilevel.eigentag = ''; %if 'largest', uses principal eigenspace, if 'smallest' uses terminal eigenspace
...
parameters.misc.MachineList = ["SVM_Linear-PCA", "SVM_Radial-PCA", "SVM_Linear", "SVM_Radial", "LogitBoost", "RUSBoost", "Bag"];
parameters.misc.PCA = true;

%% Assorted parameters
parameters.parallel.on = true;% if 1 the use parloop 
parameters.svm.kernal = []; % if 1 then use SVM with kernal, else use linear boundary 
